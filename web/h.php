...
function ajax_return($status=1,$info='请求成功',$data=array()){
    exit(json_encode(array('status'=>$status,'msg'=>$info,'data'=>$data)));
}
function redirect($addr){//跳转函数
    echo "<script type='text/javascript'>
            location.href ='".$addr."';
            </script>";
    exit;
}
$db = new cls_mysql("127.0.0.1","www_lianhebafang","njWX5mmhSF2dSiLr","www_lianhebafang");
class cls_mysql
{
    var $link_id    = NULL;
...
$appkey = "93029013";
$secret = "a190791ef13f49a39be08b25255edb98";
$username = "13860493992";
$password = "123123";
$mock = "bf3ea2428fa73434068c661106980b70ff4c8fd70";
$time = getMillisecond();
$pageSize = 10;
$pageNum = 1;

function sign(){
    $paramstring = 'appkey='.$GLOBALS['appkey'].'&password='.md5($GLOBALS['password']).'&timestamp='.$GLOBALS['time'].'&username='.$GLOBALS['username'];
    $paramstring = $GLOBALS['secret'].$paramstring.$GLOBALS['secret'];
...
//$field_values = array('access_token'=>$ticket,'add_time'=>time(),'type'=>2);
//$GLOBALS['db']->autoExecute('access_token', $field_values, "INSERT");
...
if($m=='update_cart'){
    ...
    $store_info = $db->getRow("select * from ims_yzbld_sun_stores where id = ".$store_id);
    if($store_info['is_enable']==1){
        $field_values = array("is_enable" => 0);
        $db->autoExecute('ims_yzbld_sun_stores', $field_values, "UPDATE", "id = ". $store_id);
    }else{
        $field_values = array("is_enable" => 1);
        $db->autoExecute('ims_yzbld_sun_stores', $field_values, "UPDATE", "id = ". $store_id);
    }
    ...
    ajax_return(1,'获取分类信息成功',['$result'=>$result,'$params'=>$params,'$paramstring'=>$paramstring]);
}else if($m=='login'){
    $url = "http://api.shangaoan.com/ssoapi/v2/login/login";
    $params = array(
        'appkey' => $GLOBALS['appkey'],
        'username' => $GLOBALS['username'],
        'password' => md5($GLOBALS['password']),
        'timestamp' => $time,
        'topSign' => $sign,
    );
    $paramstring = http_build_query($params);
    ...
    ajax_return(1,'登录',['$result'=>$result,'$paramstring'=>$paramstring]);
}else if($m=='sign'){
    $sign = sign();
    ajax_return(1,'签名',['sign'=>$sign,]);
}


ajax_return(0,'未匹配到路由');
 ?>