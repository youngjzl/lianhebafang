...
class UserController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $share = Share::findOne(['user_id' => \Yii::$app->user->identity->parent_id, 'status' => 1, 'is_delete' => 0]);
...
//            'parent' => $share ? $share->name : ($parent ? $parent->nickname : '总店'),
        ...
        //固定个人中心版块儿（后期要改）
        $lists=array(
            array('men_name'=>'福利社','data'=>array()),
            array('men_name'=>'功能','data'=>array()),
            array('men_name'=>'服务中心','data'=>array())
        );
        foreach ($user_center_data['menus'] as $k=>$lis){
            foreach ($lists as $k=>$datas){
                if ($datas['men_name']=='福利社'){
                    if ($lis['name']=='优惠券' || $lis['name']=='领券中心'|| $lis['name']=='抢红包' || $lis['name']=='积分兑换'){
                        array_push($lists[$k]['data'],$user_center_data['menus'][$k]);
                    }
                }
                if ($datas['men_name']=='功能'){
                    if ($lis['name']=='收货地址' || $lis['name']=='我的收藏'|| $lis['name']=='我的评价'){
                        array_push($lists[$k]['data'],$user_center_data['menus'][$k]);
                    }
                }
                if ($datas['men_name']=='服务中心'){
                    if ($lis['name']=='关于我们' || $lis['name']=='联系我们'|| $lis['name']=='服务中心'|| $lis['name']=='在线客服'){
                        array_push($lists[$k]['data'],$user_center_data['menus'][$k]);
                    }
                }
            }
        }
        $user_center_data['menus']=$lists;//--固定个人中心版块儿end
