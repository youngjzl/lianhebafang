...
class ShopListForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        $list = $query->limit($pagination->limit)->offset($pagination->offset)
            ->select('m.id,m.name,m.logo,m.recommend_status')
            ->orderBy('recommend_status desc')
            ->asArray()
            ->all();
        //收藏最多的数据
        $favorite_list=array();
        $favorite=Favorite::findBySql('SELECT goods_id,count( * ) AS count  FROM hjmallind_favorite  GROUP BY goods_id  ORDER BY count DESC LIMIT 2')->asArray()->all();
        $favorite_list = Goods::find()->select('id,name,cover_pic')->where([
            'in', 'id', [$favorite[0]['goods_id'], $favorite[1]['goods_id']]
        ])->asArray()->all();//--end收藏最多的数据
