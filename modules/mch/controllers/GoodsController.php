...
class GoodsController extends Controller
{
     ...
     * @param int $id
     */
	public function actionGoodsUpdate($id = 0)
    {   
      	$goods = Goods::findOne(['id' => $id, 'store_id' => $this->store->id, 'mch_id' => 0]);
		...
		$content = $this->juhecurl($url, $paramstring);//echo $content;
		$result = json_decode($content, true);
      	if($result){
			if($result['success'] == true){
                ...
                $goods->name = $result['data']['name'];
                $goods->price = $result['data']['price'];
                $goods->cost_price = $result['data']['mktPrice'];
                $goods->original_price = $result['data']['price'];
                $goods->cover_pic = $result['data']['thumbnail'];
                $goods->detail = $result['data']['intro'] != null ? $result['data']['intro'] : $result['data']['name'];//商品描述
                $goods->goods_num = $result['data']['enableStore'];
              	$goods->unit = $result['data']['unit'];
				$goods->sn = $result['data']['sn'];
				$goods->status = $result['data']['isSupply'];
    ...
    public function actionBatch()
    {
        ...
        } elseif ($get['type'] == 99) { //批量更新
          	$login = $this->access_token();
        	$time = $this->getMillisecond();
      		//获取商品详细
			$url = 'http://api.shangaoan.com/api/v2/goods/getGoodsInfo';
          foreach ($goods_group as $index => $value) {
          	$goods = Goods::findOne(['id' => $value['id'], 'store_id' => $this->store->id, 'mch_id' => 0]);
            $params = array(
                'accountId' => $login['accountId'],
                'memberId' => $login['memberId'],
                'token' => $login['token'],
