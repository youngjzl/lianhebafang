            ...
            <table class="table table-bordered">
                ...
                <?php foreach ($list as $item) : ?>
                    <tr>
                        ...
                        <td>
                            <?php if ($item['recommend_status'] == 1) : ?>
                            <label class="switch-label">
                                <input class="is_recommend" type="checkbox" name="is_recommendopen" checked data-id="<?= $item['id'] ?>">
                                <span class="label-icon"></span>
                                <span class="label-text"></span>
                            </label>
                            <?php else : ?>
                                <label class="switch-label">
                                    <input class="is_recommend" type="checkbox" name="is_recommendopen" data-id="<?= $item['id'] ?>">
                                    <span class="label-icon"></span>
                                    <span class="label-text"></span>
                                </label>
                            <?php endif; ?>
...
<script>
    ...
    $('input[name=is_recommendopen]').on('click', function () {
        var id = $(this).attr('data-id');
        $.loading();
        $.ajax({
            url: '<?=Yii::$app->urlManager->createUrl(['mch/mch/index/recommend'])?>',
            dataType: 'json',
            data: {
                id: id,
                status:status,
